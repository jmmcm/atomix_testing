...
%      * dllAvg: sets if averaging of DLL is on or off
...
if ~isfield(options,'diffMethod'); options.diffMethod = 'method 2'; end
...
%% Initialize (Sizes will change ig
...
for bb = 1:NB
    ...
    for zz = 1:NZ
        ...
        % Get subset of bins if close to the boundaries
        binsToBot = (zz-1);
        binsToTop = (NZ-zz);
        ...
        for tt = 1:NT
            ...
            if ~isempty(binPairsIn)
                ...
                lev3.BIN_L(tt,zz,bb,indR) = binPairsOut(:,1); 
                lev3.BIN_U(tt,zz,bb,indR) = binPairsOut(:,2);
                lev3.DLL(tt,zz,bb,indR) = dll;
                lev3.DLL_N(tt,zz,bb,indR) = dll_n; 
