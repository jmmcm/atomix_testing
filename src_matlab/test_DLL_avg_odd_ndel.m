...
matFileJMM = [dataDir,dataFileRoot,'_',processID,'_fromL2qc.mat'];
...
DLL.calcBDS = 0.5*(nanmean(DeltaLoSq+DeltaHiSq)); % BDS method
...
%%
figure(2),clf
set(gcf,'Position',[300 450 900 600])
subplot(411)
...
set(gca,'Position',[pos(1)-0.05 pos(2) 0.6 pos(4)])
ylabel('[m/s]')
ylim([-0.1 0.5])
...
legend(['\Delta_{lo}^2 = (v_{',num2str(binU2),'} - v_{',num2str(binL2),'})^2 (<> = ',num2str(nanmean(DeltaLoSq)),')'],...
    ['\Delta_{hi}^2 = (v_{',num2str(binU1),'} - v_{',num2str(binL1),'})^2 (<> = ',num2str(nanmean(DeltaHiSq)),')'],...
...
legend(['  \Delta_{lo}^2 + \Delta_{hi}^2 '],...
