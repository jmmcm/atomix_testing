...
dataPlotOptions(dd).processIDcen = 'JMM_M2aC_RM2';
...
dataPlotOptions(dd).processIDcen = 'JMM_M2aC_RM0p3';
...
dataPlotOptions(dd).processIDcen = 'JMM_M2aC_RM7p5';
...
dataPlotOptions(dd).processIDcen = 'JMM_M2aC_RM0p75';
...
dataPlotOptions(dd).processIDcen = 'JMM_M2aC_RM5';
...
for ii = 1:length(dataSets)
    ...
    EPSICen = dataCen(ii).L4.EPSI;
    EPSIAll = dataAll(ii).L4.EPSI;
    
    ind = find(dataCen(ii).L4.EPSI_FLAGS>flagLimit);
    EPSICenF = EPSICen;
    EPSICenF(ind) = NaN;
    
    ind = find(dataAll(ii).L4.EPSI_FLAGS>flagLimit);
    EPSIAllF = EPSIAll;
    EPSIAllF(ind) = NaN;
    ...
    switch ii
        case 1
            optsScatter.limits = [1e-11 1e-8];
        case 2
            optsScatter.limits = [1e-8 1e-5];
        case 3
            optsScatter.limits = [1e-8 1e-5];
        case 4
            optsScatter.limits = [1e-7 1e-4];
        case 5
            optsScatter.limits = [1e-6 1e-3];
        case 6
            optsScatter.limits = [1e-6 1e-3];
    end
    ...
    optsHist.xlabelStr = ['log_{10}(\epsilon_{All}/\epsilon_{AllAvg})'];
...
xlabel(ax(16),'\epsilon_{AllAvg} [W/kg]')
xlabel(ax(17),'log10(\epsilon_{All}/\epsilon_{AllAvg})')
xlabel(ax(18),'log10(\epsilon_{All}/\epsilon_{AllAvg})')



warning('Hacked to replace Cen with allavg'), pause