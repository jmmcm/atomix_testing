...
for ii = 1:length(dataSets)
    ...
    box on
        ylabel(ax(axNum),{dataPlotOptions(ii).dataSetSN,'\epsilon_{All}'})
...
saveas(gcf,[figDir,'Statistics.png']);

%% One site
figure(31),clf
set(gcf,'Position',[10 10 1200 400])
for ii = 6%1:length(dataSets)
    eCen = dataCen(ii).L4.EPSI(:);
    eAll = dataAll(ii).L4.EPSI(:);
    varName = 'EPSI';
    
    axNum = 1;        
    ax(axNum) = subplot(1,3,axNum);
    optsScatter.type = 'density';
    optsScatter.legend = 0;
    optsScatter.limits = [1e-6 1e-3];
    [ax(axNum),ph] = plot_EPSI_scatter(ax(axNum),eCen,eAll,optsScatter);
...
xlabel(ax(1),'\epsilon_{Cen} [W/kg]')
xlabel(ax(2),'log10(\epsilon_{All}/\epsilon_{Cen})')
xlabel(ax(3),'log10(\epsilon_{All}/\epsilon_{Cen})')
...
for ii = 1:length(dataSets)
    ...
    axNum = ii;
    ax(axNum) = subplot(2,3,axNum);
    plotData.values = eAll./eCen;
    plotData.var = 'EPSI';
    optsHist = struct();
    optsHist.nbins = 100
    optsHist.xlimits = [-1,1];  
    ...
    if ii >3
        xlabel(ax(axNum),'log10(\epsilon_{All}/\epsilon_{Cen})')
    else
        set(ax(axNum),'xticklabel',[])
    end
    title(dataSetsSN{ii})
end
